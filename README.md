...
| **P1** | 数据获取：契约探测页 + 容错解析 + 爬取队列 + 传输层 | ✅ 代码完成（实时 WebView 会话已接入 feed）；**请求配方待实机确认** |
...
**运行时行为**：进入插件会创建一个 1px 高的 Shadertoy 会话 WebView，等待 Cloudflare
挑战完成后，用同一会话动态抓取列表与详情；渲染 WebView 只消费已解析记录。网络失败时
自动退回本地缓存，缓存页仍可单独清理。请求配方仍需在真实会话中用「契约探测」确认。
