...
/** 通道绑定。字段名依据 shadertoy Rust crate 的 `RenderPassInput` 定义。 */
...
export interface ShaderRecord {
  ...
  name: string;
  username: string;
  /** 拿不到就是 null —— 不要用 0 冒充 "0 个赞"，两者含义不同。 */
  likes: number | null;
  views: number | null;
  /** Shadertoy 给的是 unix 秒的字符串。 */
  date: string;
  /** image pass 的代码。 */
  code: string;
  /** Common tab 的代码；没有则空串。 */
  common: string;
  passCount: number;
  /** 是否含 buffer pass —— D3 的复核统计就靠这个。 */
  hasBuffers: boolean;
  /** 是否声明了 `#version 300 es`。 */
  isGLSL3: boolean;
  channels: ShaderChannel[];
  /** 非 image/texture 类的通道类型（keyboard/audio/video/cubemap...），用于统计与筛选。 */
...
function unwrapShader(parsed: unknown): Record<string, unknown> | null {
...
function passType(pass: Record<string, unknown>): string {
  return asText(pass.type).trim().toLowerCase();
}

function readChannels(pass: Record<string, unknown>): {
  channels: ShaderChannel[];
  unsupported: string[];
} {
  const raw = pass.inputs;
  const channels: ShaderChannel[] = [];
  ...
  for (const item of raw) {
    ...
    const inp = item as Record<string, unknown>;
    ...
    const sampler =
      ...
      !Array.isArray(inp.sampler)
        ? (inp.sampler as Record<string, unknown>)
        : {};
    channels.push({
 ...
 */
export function parseShader(jsonText: string): ParseResult {
  let parsed: unknown;
  try {
  ...
  const info =
    ...
    !Array.isArray(shader.info)
      ? (shader.info as Record<string, unknown>)
      : {};
...
function looksLikeShaderId(value: string): boolean {
 ...
 * 递归深度有限，避免在大响应上失控。
 */
export function extractShaderIds(jsonText: string): string[] {
  let parsed: unknown;
  try {
    parsed = JSON.parse(jsonText);
  } catch {
 ...
 */
export function isSinglePassRenderable(record: ShaderRecord): boolean {
  return (
    record.passCount === 1 &&
    !record.hasBuffers &&
