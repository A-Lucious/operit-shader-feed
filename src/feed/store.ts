...
/** shader 元数据：不含代码，所以 index.json 能保持很小。 */
export interface ShaderMeta {
  ...
  hasBuffers: boolean;
  isGLSL3: boolean;
  /** 是否符合 D3 的单 pass 范围 —— 存起来，避免每次列表都重算。 */
  singlePass: boolean;
  savedAt: number;
}
...
export interface StoreOptions {
  root: string;
  quotaBytes?: number;
  /** URL → 短 hash。真实实现用 cryptojs 的 sha256；测试注入确定性实现。 */
  hash: (input: string) => string;
  /** 可注入的时钟，便于测 LRU。 */
  now?: () => number;
  /**
   * 被吞掉的错误（清理失败、淘汰失败等）走这里，而不是无声消失。
   * 纹理删不掉 = 配额可能永久超标，这种事必须能被看到。
   */
...
export function createStore(fs: StoreFs, options: StoreOptions) {
  ...
  interface IndexFile {
    version: number;
    shaders: Record<string, ShaderMeta>;
  }
  ...
  function texturePath(hash: string, ext: string): string {
   ...
   */
  async function evictImpl(): Promise<number> {
    ...
    for (const entry of entries) {
      if (textureBytes <= quotaBytes) break;
      try {
  ...
  return {
    ...
    async saveShader(record: ShaderRecord, singlePass: boolean): Promise<void> {
      await ensureDirs();
    ...
    async loadShader(id: string): Promise<ShaderRecord | null> {
      const raw = await readJson<ShaderRecord | null>(shaderPath(id), null);
      return raw && raw.id ? raw : null;
    },

    async hasShader(id: string): Promise<boolean> {
      const idx = await loadIndex();
      return hasOwn(idx.shaders, id);
    },

    /** 按保存时间倒序返回元数据 —— 这就是「拔网后仍能刷已缓存内容」的数据来源。 */
    async listShaders(): Promise<ShaderMeta[]> {
      const idx = await loadIndex();
    ...
    async recordTexture(
      ...
      const entry: TextureLedgerEntry = {
        hash,
        url,
        file,
    ...
    async touchTexture(url: string): Promise<TextureLedgerEntry | null> {
      const led = await loadLedger();
    ...
    async usage(): Promise<StoreUsage> {
      ...
      try {
        for (const entry of await fs.list(shaderDir)) {
          if (!entry.isDir) shaderBytes += entry.size;
        }
      } catch {
        shaderBytes = 0;
      }
    ...
    texturePathFor(url: string, ext: string): string {
