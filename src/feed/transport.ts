...
/**
 * 请求配方。**契约探测的输出就填在这里**（见 PLAN §6 P1a）。
 * 默认值来自公开资料确认过的内部接口：站点自己的 XHR 是 `POST /shadertoy`，
 * 带 `s=<id>`；探测页会用真实会话确认或推翻它。
 */
export interface TransportRecipe {
  list(cursor: string, limit: number): HttpRequestSpec;
  detail(id: string): HttpRequestSpec;
}
...
export interface SessionBridge {
  ...
  /** 注册结果回调；返回取消函数。 */
  onResult(
    callback: (requestId: string, ok: boolean, payload: string) => void,
  ): () => void;
}

export interface TransportOptions {
  /** 单次请求超时（毫秒）。bridge 不回调时靠它兜底，必须给。 */
  timeoutMs?: number;
  /** 结果编码解码（默认 JSON 字符串）。 */
  now?: () => number;
}
 ...
 */
export function buildFetchScript(
  requestId: string,
...
export function createSessionTransport(
  ...
  recipe: TransportRecipe = defaultRecipe(),
  ...
  return {
    async listIds(cursor: string, limit: number): Promise<ListPage> {
  ...
  } as CrawlTransport & { dispose(): void };
}
...
function extractIds(text: string): string[] {
  ...
  function push(value: unknown): void {
    ...
    if (id.length < 3 || id.length > 16) return;
  ...
  function walk(node: unknown, depth: number): void {
    ...
    if (obj.id !== undefined) push(obj.id);
    if (obj._id !== undefined) push(obj._id);
    for (const key of [
      "Results",
      "Items",
      "Shaders",
