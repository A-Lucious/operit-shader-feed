...
/** 账本状态。三态是刻意的：idle 与 pending 都说不出"有没有错"。 */
export type CompileLedgerState =
  | { kind: "idle" }
  | { kind: "pending"; since: number; codeLength: number }
  | {
      kind: "settled";
      ok: boolean;
      errors: string[];
      at: number;
      codeLength: number;
      /** 第几次编译（同一个聊天框连续编译时递增），便于 AI 判断自己是不是读到了新结果。 */
      seq: number;
    };

export interface CompileLedger {
  /** UI 把新代码下发给渲染器时调。之后 describe() 一律说「还在编译」。 */
  markPending(input?: { at?: number; codeLength?: number }): void;
  /** 渲染器回执时调 —— 成功与失败都要调（成功也要清掉 pending）。 */
  record(input: {
    ok: boolean;
    errors?: string[];
    at?: number;
  ...
  /** 给 AI 读的那段文本。永远返回一句能用的话，不会返回空串。 */
  describe(): string;
  state(): CompileLedgerState;
  reset(): void;
}

export interface CompileLedgerOptions {
  now?: () => number;
  /** 报错原文超过这个长度就截断（工具结果会进 AI 的上下文，不能无限大）。 */
  maxErrorChars?: number;
}

const DEFAULT_MAX_ERROR_CHARS = 4000;

export function createCompileLedger(
  ...
  function lengthNote(len: number): string {
