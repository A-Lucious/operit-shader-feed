...
import type { CompileLedger } from "./compile-ledger.js";

/** 界面 → main 的消息。 */
export type CompileIpcWrite =
  | { kind: "pending"; codeLength?: number }
  | { kind: "result"; ok: boolean; errors: string[]; codeLength?: number };

/** 解析后的上报对象。JSON.parse 的返回值在边界处收窄成这个形状。 */
type ParsedReport = Record<string, unknown>;
...
function parseReport(report: unknown): ParsedReport | null {
  ...
  return value && typeof value === "object" ? (value as ParsedReport) : null;
}
 ...
 */
export function toCompileIpcPayload(
  report: unknown,
  context: { codeLength?: number } = {},
): CompileIpcWrite | null {
...
export interface CompileIpcHandlers {
  /** main 侧：收界面的回执。 */
  write(payload: unknown): void;
  /** main 侧：给工具读。永远返回一段可用文本。 */
  read(): string;
}

export function createCompileIpcHandlers(
  ...
  return {
    write(payload: unknown) {
      ...
      const errors = Array.isArray(p.errors)
        ? p.errors.filter((line): line is string => typeof line === "string")
        : [];
