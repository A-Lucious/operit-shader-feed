...
const TEX_PNG_BASE64 = "iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAYAAACp8Z5+AAAAH0lEQVR4nGP4z8DwPyAgAI4ZkDkoAgz/GTBVAHX/BwARax8enytO9AAAAABJRU5ErkJggg==";
...
function cannedTransport(payloads) {
    let cursor = 0;
    return {
        async listIds() {
...
async function runSelfTest(deps) {
    const steps = [];
    ...
    const realStore = (0, store_js_1.createStore)(deps.fs, {
        root: realRoot,
        hash: deps.hash,
    });
    ...
    function finish() {
        ...
        const lines = steps.map((s) => (s.ok ? "✓ " : "✗ ") + s.name.padEnd(width) + "  " + s.detail);
    ...
    const store = (0, store_js_1.createStore)(deps.fs, {
        root: testRoot,
        hash: deps.hash,
        now: () => tickClock(),
        onWarn: (message) => {
    ...
    try {
        ...
        return fail("清理自测目录", String(err instanceof Error ? err.message : err));
    }
    step("建目录 + 首次读取", true, "clearAll 未抛异常");
    // ---- 2. 写一条真实记录并读回（验证 Tools.Files 文本往返）----
    const crawler = (0, crawler_js_1.createCrawler)(cannedTransport(exports.CANNED), {
        maxAttempts: 1,
    });
    ...
    try {
        ...
        return fail("写入 shader 正文", String(err instanceof Error ? err.message : err));
    }
    const loaded = await store.loadShader(first.id);
    const roundTrip = !!loaded &&
        loaded.code === first.code &&
        loaded.username === first.username;
    step("写入并读回 shader 正文", roundTrip, roundTrip
    ...
    step("索引列出全部记录", list.length === 3, "共 " + list.length + " 条；最新的是 " + (list[0] ? list[0].id : "无"));
    ...
    try {
        ...
        step("写入纹理并读回台账", !!touched && wroteRealBytes, "hash=" + entry.hash.slice(0, 12) + " 记账 " + entry.bytes + " 字节");
        // 去重：同 URL 再来一次不应产生第二份文件
        await store.recordTexture(textureUrl, TEX_PNG_BASE64, "png");
        const usage = await store.usage();
        step("同一 URL 的纹理去重", usage.textureCount === 1, "台账 " + usage.textureCount + " 条");
    }
    catch (err) {
    ...
    // ---- 5. 占用统计（清理入口要显示的东西）----
    const usage = await store.usage();
    const usageOk = usage.shaderCount === 3 && usage.textureCount === 1 && usage.totalBytes > 0;
    step("占用统计可用", usageOk, "shader " +
        usage.shaderCount +
        " 条 / 纹理 " +
        usage.textureCount +
        " 张 / 合计 " +
        ...
        " 字节");
    // ---- 6. feed：自动上滑 + 交给渲染器 ----
    const feedCrawler = (0, crawler_js_1.createCrawler)(cannedTransport(exports.CANNED), {
        maxAttempts: 1,
    });
    await feedCrawler.ensure(3);
    let t = 0;
    const feed = (0, feed_js_1.createFeed)(feedCrawler, {
        dwellMs: DWELL_MS,
        now: () => t,
        sessionSeed: 7,
    });
    ...
    let load = await deps.loadIntoRunner(started.current, feed.timeOffsetSeconds());
    step("渲染器接受第一条", load.ok, load.ok
        ? "iTime 起点 " + feed.timeOffsetSeconds() + "s"
        : load.error || "未知错误");
    // 快进过 30 秒，验证自动上滑与第二条加载
    t += DWELL_MS;
    const advanced = feed.tick();
    step("30 秒到点自动上滑", advanced.advanced && !!advanced.current, advanced.current ? "换到 " + advanced.current.id : "没有上滑");
    if (advanced.current) {
        load = await deps.loadIntoRunner(advanced.current, feed.timeOffsetSeconds());
        step("渲染器接受第二条", load.ok, load.ok ? advanced.current.id : load.error || "未知错误");
    }
    ...
    try {
        ...
        step("清理自测目录", false, String(err instanceof Error ? err.message : err));
    }
    const realUsageAfter = await realStore.usage();
    const untouched = JSON.stringify(realUsageBefore) === JSON.stringify(realUsageAfter);
    step("用户真实缓存未被触碰（前后占用完全一致）", untouched, untouched
