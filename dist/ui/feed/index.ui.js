...
const crawl_probe_js_1 = require("../../feed/crawl-probe.js");
const host_fs_js_1 = require("../../feed/host-fs.js");
const store_js_1 = require("../../feed/store.js");
const parse_js_1 = require("../../feed/parse.js");
...
function Screen(ctx) {
    ...
    const [target, setTarget] = ctx.useState("target", "runner");
    const [statusText, setStatusText] = ctx.useState("statusText", "正在装载资源…");
    const [pageError, setPageError] = ctx.useState("pageError", "");
    const [runnerPath, setRunnerPath] = ctx.useState("runnerPath", "");
    const [runnerScriptPath, setRunnerScriptPath] = ctx.useState("runnerScriptPath", "");
    ...
    function sendDemoShader() {
        if (flags.demoSent) {
            return;
        }
        flags.demoSent = true;
        const script = "__runnerLoad(" +
    ...
    async function startFeedOrDemo() {
        try {
            const store = ensureStore();
    ...
    function onRunnerReport(text) {
        ...
        let stage = "";
        try {
            stage = String(JSON.parse(text).stage || "");
        }
        catch {
            return;
        }
    ...
    function onRunnerSwipe(text) {
        ...
        let direction = "";
        try {
            direction = String(JSON.parse(text).direction || "");
        }
        catch {
            return;
        }
     ...
     */
    async function seedDemoCache() {
        setCacheBusy(true);
        try {
            const store = ensureStore();
            let saved = 0;
            for (const json of selftest_js_1.CANNED) {
                const parsed = (0, parse_js_1.parseShader)(json);
                if (parsed.ok && parsed.record) {
                    await store.saveShader(parsed.record, (0, parse_js_1.isSinglePassRenderable)(parsed.record));
                    saved += 1;
                }
            }
            await refreshCache("已灌入 " +
                saved +
     ...
     */
    function runCrawlProbe() {
        setStatusText("已到达 shadertoy.com，正在探测接口契约…");
        Promise.resolve(controller.evaluateJavascript(crawl_probe_js_1.CRAWL_PROBE_SCRIPT)).then((result) => {
            hostLog("[ShaderFeed] G1: evaluateJavascript 返回值 = " +
    ...
    function ensureStore() {
        if (!cacheRefs.store) {
            cacheRefs.store = (0, store_js_1.createStore)((0, host_fs_js_1.createHostFs)(), {
                root: host_fs_js_1.DEFAULT_STORE_ROOT,
                hash: host_fs_js_1.hashTextureUrl,
                onWarn: (message) => {
     ...
     * 而不是让用户面对一个莫名空白的页面。
     */
    async function refreshCache(note) {
        setCacheBusy(true);
        try {
            const store = ensureStore();
            const probe = await (0, host_fs_js_1.verifyRootWritable)(store.root);
            const usage = await store.usage();
            const lines = [];
            if (note) {
                lines.push(note, "");
            }
            lines.push(probe.message, "", usage.describe, "", "宿主能力：" + hostCapabilityLine());
    ...
    async function runDeviceSelfTest() {
        setCacheBusy(true);
        setCacheReport("正在自测…（只写 " +
            host_fs_js_1.DEFAULT_STORE_ROOT +
            "/_selftest，不动你的真实缓存）");
        try {
            const report = await (0, selftest_js_1.runSelfTest)({
                fs: (0, host_fs_js_1.createHostFs)(),
                root: host_fs_js_1.DEFAULT_STORE_ROOT,
                hash: host_fs_js_1.hashTextureUrl,
    ...
    /** 把一条记录交给渲染器。 */
    function sendShaderToRunner(record, timeOffsetSeconds) {
        if (!resourcesReady) {
        ...
        const script = "__runnerLoad(" +
            ...
            ");";
        return Promise.resolve(controller.evaluateJavascript(script)).then((result) => {
            ...
            return { ok: true };
        }, (error) => ({ ok: false, error: toErrorText(error) }));
    }
    ...
    function handleNavigation(request) {
        ...
        // 所以这个域不能丢给系统浏览器。
        if (request.url.startsWith(crawl_probe_js_1.SHADERTOY_ORIGIN)) {
            return { action: "allow" };
        }
     ...
     * 那两处迟早会漂移。
     */
    function renderWebView(url, keySuffix, purpose) {
        return UI.WebView({
