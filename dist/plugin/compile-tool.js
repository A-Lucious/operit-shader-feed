...
exports.readCompileResultText = readCompileResultText;
const chat_shader_state_js_1 = require("../shared/chat-shader-state.js");
function errText(err) {
    return err instanceof Error ? err.message : String(err);
}
/** 把通道返回值变成可用文本；不可用时返回 null（调用方负责兜底措辞）。 */
function asUsableText(value) {
    if (typeof value === "string" && value.trim() !== "") {
        return value;
    }
    return null;
}
async function readCompileResultText(callIpc) {
    const failures = [];
    // 第一次：不传选项。官方文档说非 main 上下文调用默认就发给本包的 main。
    try {
        const value = await callIpc(chat_shader_state_js_1.IPC_COMPILE_READ, {});
    ...
    try {
        const value = await callIpc(chat_shader_state_js_1.IPC_COMPILE_READ, {}, { targetRuntime: "main" });
        const text = asUsableText(value);
        if (text !== null) {
            return text;
        }
